...
首页主视觉、顶部角色入口、角色说明和聊天头像统一使用用户确认的 `public/companion-modern-v4.png`。文件为原始图片的逐字节副本，不重新生成、不加载 Blender / GLB 模型。首页完整展示人物并将操作区放在图片下方；小头像只通过 CSS 裁切面部，保留原文件。原有养成数据和聊天记录保持不变。
...
悄悄话使用明确标注的“赵露思主题 AI 角色扮演”：以第一人称自然接话、回应用户情绪与当前聊天细节，不作为百科或客服式助手。首次回复说明主题 AI 身份，直接询问身份时明确非本人；不冒充艺人、不编造其私人经历、真实关系、行程或官方授权。聊天页标题和提示始终标明 AI 角色扮演、非本人。
...
用户已于 2026-09-05 明确确认取得说话人对声音克隆和聊天产品使用的同意，并选择以 MiniMax 进行推理。服务使用音色包 `VZFKSAIHT_voice_pack.zip` 中的原始 `reference.wav` 在 MiniMax 建立并激活专属音色，不播放包内固定试听，也不部署 VoxCPM；不同模型的合成效果会有差异。

两套语音 API 都通过 `MINIMAX_VOICE_ID` 选择音色，默认 `xingbanVZFKSAIHT20260905v1`，速度 1.0、音高 0，保留 AI 水印，不额外强制情绪。音色 ID 与 MiniMax 账户绑定；使用自己的 API 密钥部署时，须换成该账户已获许可的有效音色 ID。参考原声、导入记录和试合成音频只保存在私有服务目录和被忽略的 `work/` 下，不随开源代码或网站静态资源发布。

语音响应附带 `X-Xingban-Voice-ID` 和 `X-AI-Generated` 以供运维验证；上游失败时明确报错，不悄悄换回系统音色。刷新后既有聊天记录仍保留，原有临时语音缓存会重新生成。

## MVP 边界

## 影像与资料

首页及原 feed 标签页共用 `components/star-archive.tsx`，展示经过来源核对的赵露思历史活动照片与 Wikidata 人物资料，替换原有三条演示动态。照片随静态资源部署，访客不需要注册第三方平台，也不需要跳转查看图文。来源和许可链接保留用于核查。素材署名与许可见 `public/ARCHIVE-LICENSES.md`，图片不适用代码 MIT 许可。

本版是静态历史资料快照，不是微博同步、实时官宣或每日更新；不展示虚构时间、关注数或已验证官宣标志。开放素材不构成艺人、作者或品牌对产品的背书。

## MVP 使用边界

- 当前“趙露思”主题仅为非官方粉丝向产品演示，不代表本人、工作室或任何官方机构；头像为用户提供素材，正式商用前须完成姓名、肖像及素材权利核验与授权。
- AI 聊天和 AI 语音由服务端调用 MiniMax 实时生成。页面明确标注 AI 角色扮演及 AI 合成语音，音色使用同意不代表艺人或工作室对整个产品的官方背书。
